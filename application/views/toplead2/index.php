...
<div class="row mt-3">
    <div class="col md-6">
        <div class="alert alert-danger alert-dismissible fade show" role="alert">Data Top Lead <strong>berhasil </strong><?= $this->session->flashdata('flash2');?>
...
<div class="row mt-3">
    <div class="col md-6">
        <div class="alert alert-success alert-dismissible fade show" role="alert">Data Top Lead <strong>berhasil </strong><?= $this->session->flashdata('flash');?>
        <button type="submit" class="close" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        </div>
    </div>    
</div>
...
<a href="<?= base_url('top_lead2/export_excell');?>" class="btn btn-success mb-2">Export Excell</a>
...
<div class="modal fade" id="modalTambah" tabindex="-1" aria-labelledby="modalTambahLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="<?= base_url('top_lead2/tambah'); ?>" method="POST">
        <div class="modal-body">
          <div class="form-row">
            ...
            <div class="form-group col-md-6">
              ...
              <input type="text" class="form-control" id="inputGudang" name="gudang" value="<?= $this->session->userdata('gudang'); ?>" readonly>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group col-md-6">
...
<div class="modal fade" id="modalEdit" tabindex="-1" aria-labelledby="modalEditLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="<?= base_url('top_lead2/edit'); ?>" method="POST">
        <div class="modal-body">
          <div class="form-row">
            <div class="form-group col-md-6">
              <label for="inputNama">Nama</label>
            ...
            <div class="form-group col-md-6">
              <label for="inputGudang">Gudang</label>
              <input type="text" class="form-control" id="gudang" name="gudang" readonly>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group col-md-6">
...
<script>
    ...
    $('#weekending').on('change', function() {
        ...
        $.ajax({
            url: "<?= base_url(); ?>top_lead2/tampil_data2/" + weekending,
            success: function(result) {
                let results = JSON.parse(result);
                let data = "";

                if (results[0] == "") {
                    data = `<tr><td class="text-center" colspan="13">Tidak ada data ditampilkan. Silahkan pilih tanggal weekending untuk menampilkan data.</td></tr>`;
                } else {
                    for(let i = 0; i < results.length; i++) {
                        ...
                        <a href="<?= base_url();?>top_lead2/hapus/${results[i].id}" class="btn btn-danger mt-2" style="margin-left:35px" onclick="return confirm('Yakin ingin dihapus?');"><i class="fa fa-trash"></i>Hapus</a>
                        </div>
                        </div>
                        </td>
                        </tr>
  ...
  function getDataById(id) {
    let url = "<?= base_url(); ?>top_lead2/getDataById/" + id;
